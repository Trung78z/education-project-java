...
public class ProductController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.getWriter()
                    .write(objectMapper.writeValueAsString(new ResponseWrapper<>(true, 201, product)));
            ...
            response.getWriter()
                    .write(objectMapper.writeValueAsString(new ResponseWrapper<>(500, "Internal Server Error")));
            e.printStackTrace();
        }
    }
        ...
        if (pathParts.length > 2) {
            try {
                ...
                if (pathParts[2].equals("overview")) {
                    ProductOverview productOverviewData = gson.fromJson(reader, ProductOverview.class);
                    Product updatedProduct = productService.updateProductOverview(productId, productOverviewData);
                    response.setStatus(HttpServletResponse.SC_OK);
                    ...
                    List<ProductComfortConvenience> productComfortConvenienceData = gson.fromJson(reader,
                            listType);
                    Product updatedProduct = productService.updateProductComfortConvenience(productId,
                    ...
                    Product updatedProduct = productService.updateProductSafety(productId, productSafetyData);
                    response.setStatus(HttpServletResponse.SC_OK);
                    response.getWriter().write(objectMapper.writeValueAsString(
                            new ResponseWrapper<>(true, 200, updatedProduct)));
                    ...
                    Product updatedProduct = productService.updateProductInterior(productId, productInteriorData);
                    response.setStatus(HttpServletResponse.SC_OK);
                    response.getWriter().write(objectMapper.writeValueAsString(
                            new ResponseWrapper<>(true, 200, updatedProduct)));
                    ...
                    Product updatedProduct = productService.updateProductDimensionsCapacity(productId,
                            productDimensionsCapacityData);
                    response.setStatus(HttpServletResponse.SC_OK);
                    response.getWriter().write(objectMapper.writeValueAsString(
                            new ResponseWrapper<>(true, 200, updatedProduct)));
                } else if (pathParts[2].equals("engineAndTransmission")) {
                    ProductEngineAndTransmission productEngineAndTransmissionData = gson.fromJson(reader,
                            ProductEngineAndTransmission.class);
                    Product updatedProduct = productService.updateProductEngineAndTransmission(productId,
                ...
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                response.getWriter().write(objectMapper.writeValueAsString(
                        new ResponseWrapper<>(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Invalid product ID")));
            } catch (RuntimeException e) {
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                response.getWriter()
                        .write(objectMapper.writeValueAsString(new ResponseWrapper<>(400, e.getMessage())));
            ...
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            response.getWriter().write(objectMapper.writeValueAsString(
                    new ResponseWrapper<>(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Product ID is required")));
        }
    }
